theory Dedekind_Sums
  imports Complex_Main "HOL-Number_Theory.Number_Theory" "Bernoulli.Bernoulli_FPS"
          "HOL-Library.Periodic_Fun" Real_Mod Library_Extras
begin

(* TODO: could easily be moved to the AFP by itself -- Manuel *)

section \<open>Properties of Dedekind Sums\<close>

definition dedekind_sum :: "[int, int] \<Rightarrow> real" where
...
proof -
  ...
  qed auto
  also have "\<dots> = (\<Sum>r\<in>{1..<k}. \<langle>of_int r / of_int k\<rangle>)"
    by (rule sum.reindex_bij_betw[OF bij_betw_mult_mod_int'[OF assms]])
  also have "\<dots> = 0"
...
proof -
  ...
  also have "\<dots> = (\<Sum>r = 1..<k. \<langle>real_of_int (h * r) / real_of_int k\<rangle> * \<langle>real_of_int r / real_of_int k\<rangle>)"
    by (rule sum.reindex_bij_betw [OF bij_betw_mult_mod_int']) fact
...
section \<open>The Reciprocity Law for Dedekind Sums\<close>
...
section \<open>Congruence Properties of Dedekind Sums\<close>
...
proof -
  ...
  have "[dedekind_sum' h k = h * (k - 1) * (2 * k - 1) - 0 - 0] (mod 3)"
    unfolding eq' by (intro cong_diff cong_refl) (auto simp: cong_def)
...
proof -
  ...
  also have "[\<dots> = h * (k - 1) * (1 * k + 1)] (mod 3)"
    by (intro cong_mult cong_refl cong_add cong_diff) (auto simp: cong_def)
...
next
  case equal
  thus ?thesis using assms by (auto simp: cong_def)
qed
...
proof -
  ...
  proof (intro cong_sum, goal_cases)
    ...
    proof (cases "odd r")
      ...
      have "[4 * r * \<lfloor>real_of_int (h * r) / k\<rfloor> = 4 * \<lfloor>real_of_int (h * r) / k\<rfloor>] (mod 8)"
        by (rule cong_mult[OF _ cong_refl]) (use * in \<open>auto simp: cong_def\<close>)
      thus ?thesis
  ...
  also have "[\<dots> = -2 * h * (k - 1) + h * 0 - 4 * S1 + k * (k - 1) - 0] (mod 8)"
    using dvd by (intro cong_diff cong_add S12 cong_mult cong_refl)
                 (auto simp: cong_def mod_eq_0_iff_dvd)
...
proof -
  ...
  proof -
    ...
    have "[k ^ 2 = ((2 * k') mod (2 * 4) + 1) ^ 2 mod 8] (mod 8)"
      unfolding k' by (intro cong_add cong_refl cong_pow cong_mod_rightI) (auto simp: cong_def)
...
proof -
  ...
  have "[a * d - b * 0 = a * d - b * c] (mod c)"
    by (intro cong_diff cong_mult cong_refl) (auto simp: cong_def)
  also have "a * d - b * c = 1"
    by fact
